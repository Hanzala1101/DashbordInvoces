...
export class MonitoringComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.invoiceStore.state$.subscribe((state) => {
      this.seltotal = state?.data?.filter(
        (item: any) => item.PRTF === 'OIS199PF',
      ).length;
      this.mrototal = state?.data?.filter(
        (item: any) => item.PRTF === 'COS184PF',
      ).length;
  ...
  calculateCompleted(data: any[]): void {
    this.selcompleted = data.filter(
      (item: any) => item.PRTF === 'OIS199PF' && item?.INST > 75,
    ).length;
    this.mrocompleted = data.filter(
      (item: any) => item.PRTF === 'COS184PF' && item?.INST > 75,
    ).length;
  }

  get selpercentage(): number {
    return Math.round((this.selcompleted / this.seltotal) * 100) || 0;
  }
  get mropercentage(): number {
    return Math.round((this.mrocompleted / this.mrototal) * 100) || 0;
  }
